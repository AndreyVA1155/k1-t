...
if (shopList) {
    shopList.addEventListener('click', (evt) => {
        ...
        var priceProduct = $(prod).children('.product__price').text();
        var priceProduct1 = priceProduct.split(' ')[0];
        var priceid = document.getElementById("price");
        priceid.setAttribute('value', priceProduct1);
...
if (pageOrderList) {
    pageOrderList.addEventListener('click', evt => {
        ...
        if (evt.target.classList && evt.target.classList.contains('order-item__btn')) {
            ...
            $.post('/admin/ajax/changeOrderStatus.php', {
                id: $(status).attr('data-id'),
                statusValue: statusValue
            }, function (data) {
                console.log({id: $(status).attr('data-id'), statusValue: statusValue});
            });
        }
    });
}
...
const buttonContinueShopping = document.getElementById('buttonContinueShopping');
if (buttonContinueShopping) {
    {
        buttonContinueShopping.addEventListener('click', (evt) => {
            evt.preventDefault();
...
if (productsList) {
    ...
    productsList.addEventListener('click', evt => {
        ...
        if (target.classList && target.classList.contains('product-item__delete')) {
            ...
            $.post('/admin/ajax/deleteProduct.php', {
...
if (document.querySelector('.shop-page')) {
    var min = $('div.hidden').data('min') / 1;
    var max = $('div.hidden').data('max') / 1;
